...
with open(os.path.join(cwd, 'phones.csv'), 'r') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
        cur.execute(
        "INSERT INTO phones VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        row
    )

with open(os.path.join(cwd, 'customers.csv'), 'r') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
        cur.execute(
...
plt.savefig(os.path.join(cwd, 'new_graphs.png'))
